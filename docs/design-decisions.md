...
## 8. Citation policy (non-negotiable)
The system enforces sentence-level citations (Level-3).
Any generated sentence must be traceable to one or more indexed text chunks.
If no supporting chunk exists, the system must return “No supporting source found.”
**LLM constraints**  
- LLM is not allowed to answer outside retrieved context
- tructured output is required before formatting

---

## 9. Optional Retrieval-Augmented Generation (RAG)
